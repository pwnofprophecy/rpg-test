...
Two parallel enums, each with `NONE` as the default so existing `.tres` files load unchanged:
- **`Equipment.WeaponDamageType`** = `NONE, PIERCING, SLASHING, BLUDGEONING` — exposed via `weapon_damage_type` on `Equipment`. Only meaningful when `slot == WEAPON`; leave at `NONE` for armor/accessories (and for untyped weapons like fists).
- **`Spell.MagicDamageType`** = `NONE, FIRE, ICE, LIGHTNING, WATER, HOLY, FORCE` — exposed via `damage_type` on `Spell`. Use `NONE` for `HEAL_HP` and `CURE_STATUS` spells (they don't deal damage) and for any untyped damage spell.

Assignment is per-`.tres` in the Inspector — open a weapon or spell, pick the type from the dropdown. Adding new types later is one enum entry; consumers (the eventual damage mod, the sandbox display, etc.) read from the same enum so additions propagate automatically.
...
**Gated battle actions**: every non-Attack command is mod-gated. `battle_ui.gd` holds `_ALL_ACTIONS = ["attack","magic","item","run"]` (top→bottom list order) and `_ACTION_MODS` mapping each gated action to its required mod — `magic` → `MAGIC.EXE`, `item` → `ITEMS.EXE`, `run` → `RUN.EXE`. Attack is always present (never in `_ACTION_MODS`). `show_menu()` calls `_build_actions()`, which filters `_ALL_ACTIONS` through the gates into the runtime `_actions` list; actions whose mod isn't active are dropped. The menu is a `VBoxContainer` (`MenuBox/MenuList`) of four `Label` slots; `_update_cursor()` renders `_actions[i]` into slot `i` for the first N slots and hides any leftovers, and ALL cursor navigation / mouse clicks index into `_actions` — so a hidden Run button is genuinely absent (can't be navigated to or clicked), not just visually dimmed. Navigation is plain Up/Down with wrap-around (same as the Magic / Item sub-menus), so any count of actions works without special-casing. Because `_actions` is compacted and the VBox lays out only visible children, this generalizes: gating a *middle* action (e.g. Item later) also works — the remaining actions slide up into the upper slots and the menu's footprint shrinks naturally with no empty cells.

**Gating a new battle action behind a mod**: add a `"<action>": "<mod_id>"` entry to `_ACTION_MODS` in `battle_ui.gd`. No other code changes. (Default mod state is inactive, so until the player finds + activates the relevant `.EXE` — or toggles it in the sandbox — the gated command won't appear in any battle. With nothing active, the menu shows just Attack.)
